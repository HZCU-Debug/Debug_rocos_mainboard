...
USBH_Class_cb_TypeDef  HID_callb = 
{
...
{	
  ...
  if(pphost->device_prop.Itf_Desc[0].bInterfaceSubClass  == HID_BOOT_CODE) //子设备为HID类型
  {
    ...
    {
      if(pphost->device_prop.Ep_Desc[0][num].bEndpointAddress & 0x80) //输入端点
      {
  ...
  else if(pphost->device_prop.Itf_Desc[0].bInterfaceSubClass==HID_JOYSTICK_SUB_CLASS_CODE)//北通无线游戏手柄HID设备
  {  
        //自定义协议,支持游戏手柄
        if(pphost->device_prop.Itf_Desc[0].bInterfaceProtocol == HID_JOYSTICK_PROCOTOL_CODE)
        {
            HID_Machine.cb = &HID_JOYSTICK_cb;//游戏手柄回调函数
        }  
        HID_Machine.state     = HID_IDLE;
        HID_Machine.ctl_state = HID_REQ_IDLE; 
        HID_Machine.ep_addr   = pphost->device_prop.Ep_Desc[0][0].bEndpointAddress;
        ...
        for (num=0; num < maxEP; num++)
        {
            if(pphost->device_prop.Ep_Desc[0][num].bEndpointAddress & 0x80)//输入端点
            {
                ...
                HID_Machine.hc_num_in  =\
                USBH_Alloc_Channel(pdev, 
                                   pphost->device_prop.Ep_Desc[0][num].bEndpointAddress);//2
                                   ...
                                   void   *phost)
{
  ...
  case HID_POLL:
    ...
    else if(HCD_GetURB_State(pdev , HID_Machine.hc_num_in) == URB_DONE)
    {
      ...
      {
        start_toggle = 0;
        HID_Machine.cb->Decode(HID_Machine.buff);  //HID 数据包解析
      }
    }
