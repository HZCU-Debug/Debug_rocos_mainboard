...
USBH_Status USBH_GetDescriptor(USB_OTG_CORE_HANDLE *pdev,
                               ...
                               uint16_t length )
{ 
  phost->Control.setup.b.bmRequestType = USB_D2H | req_type; //设备到主机
  phost->Control.setup.b.bRequest = USB_REQ_GET_DESCRIPTOR;  //请求为获取描述符
  ...
  return USBH_CtlReq(pdev, phost, buff , length );   //发起控制传输   
}
