                  ...
                  USB_OTG_CORE_ID_TypeDef coreID)
{
  ...
  pdev->host.hc[0].max_packet  = 8; //默认端点0 控制端点最大数据包大小8

  USB_OTG_SelectCore(pdev, coreID);    //选择相应的OTG 内核  FS  OR  HS
#ifndef DUAL_ROLE_MODE_ENABLED
  USB_OTG_DisableGlobalInt(pdev);      //屏蔽全局中断
  USB_OTG_CoreInit(pdev);

  /* Force Host Mode*/
  USB_OTG_SetCurrentMode(pdev , HOST_MODE);    //设置成主机模式
  USB_OTG_CoreInitHost(pdev);
  USB_OTG_EnableGlobalInt(pdev);     //usb 全局中断不屏蔽 使能
#endif
