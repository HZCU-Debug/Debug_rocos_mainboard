...
uint32_t USBH_OTG_ISR_Handler (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  {
    gintsts.d32 = USB_OTG_ReadCoreItr(pdev); //读取中断标志位
    if (!gintsts.d32)  //没有发生中断或者中断被全部屏蔽
    {
      return 0;
    }
    
    if (gintsts.b.sofintr)  //sof 中断
    {
      retval |= USB_OTG_USBH_handle_sof_ISR (pdev);
    }
    
    if (gintsts.b.rxstsqlvl)//RXFIFO 非空 RX FIFO 有数据包可读取
    {
      retval |= USB_OTG_USBH_handle_rx_qlvl_ISR (pdev);
    }
    
    if (gintsts.b.nptxfempty)  //非周期TX FIFO 非空
    {
      retval |= USB_OTG_USBH_handle_nptxfempty_ISR (pdev);
    }
    
    if (gintsts.b.ptxfempty)//周期性 TX FIFO 空
    {
      retval |= USB_OTG_USBH_handle_ptxfempty_ISR (pdev);
    }    
    
    if (gintsts.b.hcintr)//主机通道中断 为1表示模块中一个通道上存在挂起的中断
    {
      retval |= USB_OTG_USBH_handle_hc_ISR (pdev);
    }
    
    if (gintsts.b.portintr)//主机端口中断 指示控制器端口的状态发生变化
    {
      retval |= USB_OTG_USBH_handle_port_ISR (pdev);
    }
    
    if (gintsts.b.disconnect)//检测到断开连接中断
    {
      retval |= USB_OTG_USBH_handle_Disconnect_ISR (pdev);  
      
    }
    
    if (gintsts.b.incomplisoout)//未完成OUT同步传输
    {
  ...
  * GINTSTS */
  ...
  for (i = 0; i < pdev->cfg.host_channels ; i++)
  {
    if (haint.b.chint & (1 << i))
    {
      ...
      if (hcchar.b.epdir) //端点方向
      {
        retval |= USB_OTG_USBH_handle_hc_n_In_ISR (pdev, i); //输入
      }
      else
      {
        retval |=  USB_OTG_USBH_handle_hc_n_Out_ISR (pdev, i);//输出
      }
    }
  }
...
static uint32_t USB_OTG_USBH_handle_Disconnect_ISR (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  USBH_HCD_INT_fops->DevDisconnected(pdev); //USB 处于断开状态
  
  /* Clear interrupt */
  gintsts.b.disconnect = 1;   //中断状态位清0
  USB_OTG_WRITE_REG32(&pdev->regs.GREGS->GINTSTS, gintsts.d32);
...
static uint32_t USB_OTG_USBH_handle_port_ISR (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if (hprt0.b.prtconndet)  //检测到端口连接
  {

    hprt0_dup.b.prtconndet = 1;
    USBH_HCD_INT_fops->DevConnected(pdev);  //usb 已经连接
    retval |= 1;
  }
  
  /* Port Enable Changed */
  if (hprt0.b.prtenchng)  //主机发送端口复位操作10ms 后 产生该中断 端口使能禁止变化
  {
    hprt0_dup.b.prtenchng = 1;
    
    if (hprt0.b.prtena == 1)//端口使能
    {
          ...
          (hprt0.b.prtspd == HPRT0_PRTSPD_FULL_SPEED))
      {
        ...
        if (hprt0.b.prtspd == HPRT0_PRTSPD_LOW_SPEED)//低速
        {
        ...
        else  //FS
        {
          ...
          USB_OTG_WRITE_REG32(&pdev->regs.HREGS->HFIR, 48000 );   //1ms         
  ...
  if (hprt0.b.prtovrcurrchng)  //端口过流变化
  {
...
uint32_t USB_OTG_USBH_handle_hc_n_Out_ISR (USB_OTG_CORE_HANDLE *pdev , uint32_t num)
{
  ...
  if (hcint.b.ahberr)  //AHB总线错误 适用于HS
  {
    CLEAR_HC_INT(hcreg ,ahberr);
    UNMASK_HOST_INT_CHH (num);
  } 
  else if (hcint.b.ack)  //收到发出ACK响应
  {
    CLEAR_HC_INT(hcreg , ack);
  }
  else if (hcint.b.frmovrun) //帧溢出错误
  {
    UNMASK_HOST_INT_CHH (num);
    USB_OTG_HC_Halt(pdev, num);
    CLEAR_HC_INT(hcreg ,frmovrun);
  }
  else if (hcint.b.xfercompl) //传输完成
  {
  ...
  else if (hcint.b.stall)//收到STALL响应
  {
  ...
  else if (hcint.b.xacterr)  //通信事务错误
  {
    UNMASK_HOST_INT_CHH (num);
    USB_OTG_HC_Halt(pdev, num);
  ...
  else if (hcint.b.chhltd) //传输非正常结束
  {
