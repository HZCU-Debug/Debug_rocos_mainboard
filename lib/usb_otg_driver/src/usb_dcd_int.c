...
uint32_t USBD_OTG_ISR_Handler (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  {
    gintr_status.d32 = USB_OTG_ReadCoreItr(pdev); //返回模块中断状态寄存器值
    if (!gintr_status.d32) /* avoid spurious interrupt */
    {
      return 0;
    }
    
    if (gintr_status.b.outepintr)                 //out端点存在挂起中断 中断使能了 PC机主机下发数据给设备
    {
      retval |= DCD_HandleOutEP_ISR(pdev);
    }    
    
    if (gintr_status.b.inepint)                  //IN端点中断 中断使能了  设备上传数据给PC主机
    {
      retval |= DCD_HandleInEP_ISR(pdev);
    }
    
    if (gintr_status.b.modemismatch)            //模式不匹配 中断未使能
    {
    ...
    if (gintr_status.b.sofintr)                //SOF帧中断   中断使能了 usb设备收到一个SOF令牌
    {
      retval |= DCD_HandleSof_ISR(pdev);
      
    }
    
    if (gintr_status.b.rxstsqlvl)              //RXFIFO 非空中断  中断使能了
    {
      retval |= DCD_HandleRxStatusQueueLevel_ISR(pdev);
      
    }
    
    if (gintr_status.b.usbreset)              //usb上检测到复位信号  中断使能了
    {
      retval |= DCD_HandleUsbReset_ISR(pdev);
      
    }
    if (gintr_status.b.enumdone)               //枚举完成中断 中断使能了
    {
      retval |= DCD_HandleEnumDone_ISR(pdev);
    }
    
    if (gintr_status.b.incomplisoin)            //未完成IN同步传输 中断使能了
    {
      retval |= DCD_IsoINIncomplete_ISR(pdev);
    }

    if (gintr_status.b.incomplisoout)           //未完成OUT同步传输 中断使能了
    {
      retval |= DCD_IsoOUTIncomplete_ISR(pdev);
    }    
#ifdef VBUS_SENSING_ENABLED
    if (gintr_status.b.sessreqintr)             //检测到会话请求中断
    {
...
static uint32_t DCD_HandleInEP_ISR(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  while ( ep_intr )
  {
    if (ep_intr&0x1) /* In ITR */
    {
      ...
      if (diepint.b.emptyintr)   //IN端点发送FIFO为空中断
      {
        
        DCD_WriteEmptyTxFifo(pdev , epnum); //写数据到TXFIFO
...
static uint32_t DCD_HandleOutEP_ISR(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  while ( ep_intr )
  {
    if (ep_intr&0x1) //哪个端点号发生中断
    {
      
      doepint.d32 = USB_OTG_ReadDevOutEP_itr(pdev, epnum);
      
      /* Transfer complete */
      if ( doepint.b.xfercompl ) //数据接收完成
      {
        /* Clear the bit in DOEPINTn for this interrupt */
        ...
        /* Inform upper layer: data ready out端点数据传输完成，可以从寄存器中读取数据*/
        /* RX COMPLETE */
