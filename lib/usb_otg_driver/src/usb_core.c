...
static void USB_OTG_EnableCommonInt(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  int_mask.b.wkupintr = 1;    //远程唤醒中断不屏蔽
  int_mask.b.usbsuspend = 1; //USB 挂起中断不屏蔽
                               ...
                               USB_OTG_CORE_ID_TypeDef coreID)
{
  ...
  if (coreID == USB_OTG_FS_CORE_ID)
  {
    ...
    pdev->cfg.host_channels    = 8 ;   //FS 8个管道
  ...
  else if (coreID == USB_OTG_HS_CORE_ID)
  {
    ...
    pdev->cfg.host_channels    = 12 ;    //HS 12个管道
...
USB_OTG_STS USB_OTG_CoreInit(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if (pdev->cfg.phy_itface == USB_OTG_ULPI_PHY)//HS
  {
  ...
  else /* FS interface (embedded Phy) */
  {
    ...
    gccfg.b.vbussensingB = 1 ;       //使能A器件的VBUS感应功能
#ifndef VBUS_SENSING_ENABLED
    gccfg.b.disablevbussensing = 1;   //硬件不支持VBUS感应
#endif    
    
    if(pdev->cfg.Sof_output)  //sof 输出使能
    {
      gccfg.b.sofouten = 1;    //SOF 脉冲从引脚输出 0 SOF 脉冲不从引脚输出
    }
    
    USB_OTG_WRITE_REG32 (&pdev->regs.GREGS->GCCFG, gccfg.d32);
    USB_OTG_BSP_mDelay(20);
  }
  /* case the HS core is working in FS mode  HS*/
  if(pdev->cfg.dma_enable == 1)
  {
...
USB_OTG_STS USB_OTG_DisableGlobalInt(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  USB_OTG_GAHBCFG_TypeDef  ahbcfg;
  ahbcfg.d32 = 0;
  ahbcfg.b.glblintrmsk = 1; /* Enable interrupts 取消对全局中断的屏蔽 */
  USB_OTG_MODIFY_REG32(&pdev->regs.GREGS->GAHBCFG, ahbcfg.d32, 0);  //屏蔽全局中断
  return status;
}
...
USB_OTG_STS USB_OTG_SetCurrentMode(USB_OTG_CORE_HANDLE *pdev , uint8_t mode)
{
  ...
  USB_OTG_WRITE_REG32(&pdev->regs.GREGS->GUSBCFG, usbcfg.d32);
  USB_OTG_BSP_mDelay(50);   //等待至少25ms 后生效
  return status;
}
...
USB_OTG_STS USB_OTG_CoreInitHost(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  USB_OTG_WRITE_REG32(pdev->regs.PCGCCTL, 0); //USB恢复通信或者新会话启动，将此位清0
  ...
  {
    USB_OTG_InitFSLSPClkSel(pdev , HCFG_48_MHZ);  //PHY 时钟以48Mhz运行
  }
  USB_OTG_ResetPort(pdev);               //主机端口复位
  ...
  /* Rx FIFO */
#ifdef USB_OTG_FS_CORE   //USB RAM 1.24KB=1280Byte (128+96+96)*4=1280Byte GRXFSIZ 以32位字为单位
  if(pdev->cfg.coreID == USB_OTG_FS_CORE_ID)
  {
    ...
    USB_OTG_WRITE_REG32(&pdev->regs.GREGS->DIEPTXF0_HNPTXFSIZ, nptxfifosize.d32);//tx 非周期FIFO设置 端点0
    
    ptxfifosize.b.startaddr = RX_FIFO_FS_SIZE + TXH_NP_FS_FIFOSIZ;   //周期性 TX FIFO 以 非周期RAM 结束地址开始
    ptxfifosize.b.depth     = TXH_P_FS_FIFOSIZ;
    USB_OTG_WRITE_REG32(&pdev->regs.GREGS->HPTXFSIZ, ptxfifosize.d32);    //TX  周期FIFO设置  
  }
#endif
  ...
  USB_OTG_FlushTxFifo(pdev, 0x10000 );         /* all Tx FIFOs */
  ...
  for (i = 0; i < pdev->cfg.host_channels; i++)
  {
    USB_OTG_WRITE_REG32( &pdev->regs.HC_REGS[i]->HCINT, 0xFFFFFFFF ); //全部清0
    USB_OTG_WRITE_REG32( &pdev->regs.HC_REGS[i]->HCINTMSK, 0 );         //屏蔽中断
  }
#ifndef USE_OTG_MODE
...
{
  return !(USB_OTG_READ_REG32(&pdev->regs.HREGS->HFNUM) & 0x1);//帧时间寄存器减到0表示新的SOF 一帧开始
}
...
USB_OTG_STS USB_OTG_EnableHostInt(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  /* Clear any pending interrupts. */
  USB_OTG_WRITE_REG32(&pdev->regs.GREGS->GINTSTS, 0xFFFFFFFF);//清所有中断
  ...
  intmsk.b.portintr   = 1;    //主机端口中断不屏蔽
  intmsk.b.hcintr     = 1;    //主机通道中断不屏蔽
  intmsk.b.disconnect = 1;     //检测到端口连接中断不屏蔽
  intmsk.b.sofintr    = 1;     //SOF 帧起始中断不屏蔽
  intmsk.b.incomplisoout  = 1;  //未完成周期性传输中断不屏蔽
...
uint32_t USB_OTG_ResetPort(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  hprt0.d32 = USB_OTG_ReadHPRT0(pdev);
  hprt0.b.prtrst = 1;  //置1 端口复位
  USB_OTG_WRITE_REG32(pdev->regs.HPRT0, hprt0.d32);
  USB_OTG_BSP_mDelay (20);                                /* See Note #1 */
  hprt0.b.prtrst = 0;     //清0
  USB_OTG_WRITE_REG32(pdev->regs.HPRT0, hprt0.d32);
  USB_OTG_BSP_mDelay (20);   
  return 1;
...
USB_OTG_STS USB_OTG_HC_Init(USB_OTG_CORE_HANDLE *pdev , uint8_t hc_num)
{
  ...
  USB_OTG_HCCHAR_TypeDef     hcchar;  //主机通道特性寄存器
  ...
  {
  case EP_TYPE_CTRL:
  case EP_TYPE_BULK:   //控制传输和批量传输
    hcintmsk.b.xfercompl = 1;  //传输完成中断不屏蔽
    hcintmsk.b.stall = 1;      //stall 响应中断不屏蔽
    hcintmsk.b.xacterr = 1;    //通信事务错误不屏蔽
    hcintmsk.b.datatglerr = 1;  //数据同步错误不屏蔽
    hcintmsk.b.nak = 1;         //NAK 响应接收中断不屏蔽
    if (pdev->host.hc[hc_num].ep_is_in) 
    {
      hcintmsk.b.bblerr = 1;     //串扰错误不屏蔽
    } 
    else 
    {
      hcintmsk.b.nyet = 1;     //NYET响应接收中断不屏蔽
      if (pdev->host.hc[hc_num].do_ping) 
      {
        hcintmsk.b.ack = 1;     //ACK 响应接收中断不屏蔽
      }
    }
    break;
  case EP_TYPE_INTR:    //中断传输
  ...
  /* Enable the top level host channel interrupt. 使能对应的端点通道号中断*/
  ...
  hcchar.d32 = 0;
  hcchar.b.devaddr = pdev->host.hc[hc_num].dev_addr;//设备地址
...
USB_OTG_STS USB_OTG_HC_StartXfer(USB_OTG_CORE_HANDLE *pdev , uint8_t hc_num)
{
  USB_OTG_STS status = USB_OTG_OK;
  USB_OTG_HCCHAR_TypeDef   hcchar;  //主机通道特性寄存器
  USB_OTG_HCTSIZn_TypeDef  hctsiz;  //主机通道传输大小寄存器
  USB_OTG_HNPTXSTS_TypeDef hnptxsts; 
  USB_OTG_HPTXSTS_TypeDef  hptxsts; 
  USB_OTG_GINTMSK_TypeDef  intmsk;
  uint16_t                 len_words = 0;    //要传输的word数  32bit为单位 
  
  uint16_t num_packets;  //传输的包个数
  ...
  if (pdev->host.hc[hc_num].xfer_len > 0)
  {
    num_packets = (pdev->host.hc[hc_num].xfer_len + \
      pdev->host.hc[hc_num].max_packet - 1) / pdev->host.hc[hc_num].max_packet;//max_packet=64
    
    if (num_packets > max_hc_pkt_count) //要传输的数据不能>256包 256
    {
  ...
  /* Initialize the HCTSIZn register 初始化主机通道传输大小寄存器*/
  ...
  hcchar.b.chen = 1;  //使能主机通道
  hcchar.b.chdis = 0;
  USB_OTG_WRITE_REG32(&pdev->regs.HC_REGS[hc_num]->HCCHAR, hcchar.d32);
  
  if (pdev->cfg.dma_enable == 0) /* Slave mode */
  {  
    if((pdev->host.hc[hc_num].ep_is_in == 0) &&  //0 表示 OUT
       (pdev->host.hc[hc_num].xfer_len > 0))
    {
      ...
      case EP_TYPE_BULK:
        ...
        len_words = (pdev->host.hc[hc_num].xfer_len + 3) / 4;  //32byte 为单位
        
        /* check if there is enough space in FIFO space */
        if(len_words > hnptxsts.b.nptxfspcavail)
        {
          /* need to process data in nptxfempty interrupt */
          intmsk.b.nptxfempty = 1; //使能非周期TXFIFO 空中断屏蔽
  ...
  else
  {
    ...
    */
    ...
    else
    {
      ...
      deptsiz.b.xfersize = deptsiz.b.pktcnt * ep->maxpacket;  //当前端点一次数据要传输的数据量
    }
